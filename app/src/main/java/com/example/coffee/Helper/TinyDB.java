    ...
    public <T> ArrayList<T> getListObject(String key, Class<T> clazz) {
        ...
        for (String json : objStrings) {
            if (json == null || json.isEmpty()) {
                Log.w("TinyDB", "getListObject: Skipping null or empty JSON string for key: " + key);
                continue;
            }

            try {
                T player = new Gson().fromJson(json, clazz);
                if (player != null) {
                    playerList.add(player);
                } else {
                    Log.w("TinyDB", "getListObject: Parsed object is null for key: " + key);
                }
            } catch (Exception e) {
                Log.e("TinyDB", "getListObject: Failed to parse JSON: " + e.getMessage());
            }
        }
